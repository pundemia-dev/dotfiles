...
const float VIB_VIBRANCE = 0.1; //0.25;
const vec3 VIB_coeffVibrance = VIB_RGB_BALANCE * -VIB_VIBRANCE;

// --- НАСТРОЙКИ БАЛАНСА БЕЛОГО (УБИРАЕМ СИНЕВУ) ---
// Значения: vec3(Красный, Зеленый, Синий). 
// 1.0 = 100% цвета. Чтобы убрать синеву, уменьшаем 3-е число (например, 0.85 = 85% синего).
// Чуть-чуть уменьшаем 2-е (зеленый), чтобы белый не казался зеленоватым.
const vec3 WHITE_BALANCE = vec3(1.0, 1.0, 0.7);
...
void main() {
    ...
    // Получаем цвет после применения насыщенности
    vec3 adjustedColor = mix(vec3(luma), color, p_col);
    
    // Применяем нашу коррекцию белого (умножаем каналы)
    adjustedColor = adjustedColor * WHITE_BALANCE;
